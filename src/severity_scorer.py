...
def score_collision(
    ...
    # Centrality: normalize against a reasonable max (50 callers = max score)
    centrality_factor = min(centrality / 50.0, 1.0)
    if len(callers) > 0:
        caller_factor = min(len(callers) / 20.0, 1.0)
        centrality_factor = max(centrality_factor, caller_factor)

    # Owner spread: distinct project paths represent team boundaries
    distinct_projects = len({c.project_path for c in callers}) if callers else 1
    owner_spread_factor = min(distinct_projects / 5.0, 1.0)
    owner_spread_factor = max(owner_spread_factor, 0.2)
    ...
    change_risk = {
        ...
        ChangeType.SIGNATURE_CHANGED: 0.75,
        ChangeType.ADDED: 0.1,
    }
    risk_factor = change_risk.get(changed_symbol.change_type, 0.5)

    # Test coverage reduces risk (if CI reports coverage we trust it somewhat)
    coverage_multiplier = 0.7 if has_ci_coverage else 1.0

    raw_score = centrality_factor * owner_spread_factor * risk_factor * coverage_multiplier
